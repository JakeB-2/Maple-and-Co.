...
// The quick-log type picker — a bottom-sheet grid of event-type tiles, on the
// app's DrawerShell chrome like every other drawer.
//
// It has NO FAB of its own: the single shared shell FAB (components/shell/
// capture-fab) opens it from anywhere by routing to /pets?capture=1 (or
// /plants), which resolves to this entity-scoped picker on the profile page
// (D-033). The shell has no entity data, so it just sets the param and this
// on-page component — which DOES have the entity + its kind's types — owns
// the tiles.
//
...
import { logEntityEvent } from '@/lib/actions/entity-events'
...
export function QuickLog({
  ...
  /** Current query minus `?capture` (+ any extra mutation), as a profile href. */
  ...
  function pick(type: EventTypeRow) {
  ...
  function autoLog(type: EventTypeRow) {
    ...
    startTransition(async () => {
      ...
      toast.success(`${type.emoji} ${type.name} logged ${KIND_COPY[kind].logFlourish}`, {
        duration: 15000,
        action: {
          label: 'Undo',
          onClick: async () => {
            const undo = await softDelete('entity_events', eventId)
            if (undo.error) {
