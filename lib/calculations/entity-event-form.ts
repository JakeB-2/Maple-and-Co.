// Pure form <-> EAV mapping for entity events (D-013): the one generic
// quick-log form is built from a type's attribute rows, keyed by attribute id.
// No supabase or server imports — unit-tested directly.

import { z } from 'zod'
import type { EventTypeAttributeRow } from '@/lib/queries/event-types'
import type { EntityEventValueRow } from '@/lib/queries/entity-events'
...
function choiceIds(value: EntityEventValueRow | undefined): string[] {
  if (!value || !Array.isArray(value.choice_ids)) return []
  return value.choice_ids.filter((id): id is string => typeof id === 'string')
}

function attributeSchema(attr: EventTypeAttributeRow): z.ZodType<unknown> {
