...
export default async function MaplePage({
  ...
  const [types, attributes, events, latest, weightSeries, profiles, selected] = await Promise.all([
  ...
  const medsTypeId = types.find((type) => type.system_key === 'meds')?.id
  ...
  return (
    <div className="flex flex-col gap-6">
      <PetProfile pet={pet} chips={chips} weightSeries={weightSeries} lastMeds={lastMeds} />
